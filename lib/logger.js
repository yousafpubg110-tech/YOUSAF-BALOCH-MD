...
class Logger {
  constructor() {
    ...
    // FIX: try-catch added — crash nahi hoga agar logs folder na bane
    try {
  ...
  // FIX: logFile update karo har bar — date change hone par nayi file
  get currentLogFile() {
    return path.join(this.logDir, `${this.getDate()}.log`);
  }
  ...
  // FIX: formatMessage helper — code duplication khatam
  ...
  cleanOldLogs() {
    try {
      ...
      for (const file of files) {
        // FIX: Only delete .log files — baaki files safe rahe
        ...
        try {
          ...
          console.error(`❌ Could not delete log ${file}:`, fileErr.message);
        }
      }
    } catch (error) {
